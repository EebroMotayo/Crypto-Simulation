...
function syncToInterval(interval) {
	...
	if(interval == "Simulation"){
		...
		if(chartData0 == "Open"){
			...
			fetch('http://127.0.0.1:5000/charthistory/BTCUSDT/1d')
			.then((r) => r.json())
			.then((response) => {
		...
		if(chartData == "Open"){
			...
			fetch('http://127.0.0.1:5000/charthistory/BTCUSDT/4h')
			.then((r) => r.json())
			.then((response) => {
				//console.log(response)		
				candleSeries.setData(response);
...
// 	  resolve("I love You !!");
// 	});
// 	document.getElementById("pauseBtn").innerHTML = await myPromise;
//   }
...
// 	let myData = await fetch('http://127.0.0.1:5000/chartz_runbot_ema_slow_sim/BTCUSDT/1d');

// 	myData.then((r) => r.json()).then((response) => {
...
async function fetchProducts() {
	try {
	  ...
	  const response = await fetch('http://127.0.0.1:5000/chartz_runbot_ema_slow_sim/BTCUSDT/1d');
	  if (!response.ok) {
...
async function load_slowNoiseEMA2(){
	...
	try {
		...
		const response = await fetch('http://127.0.0.1:5000/chartz_runbot_ema_slow_sim/'+symbl+'/'+tmfrm);
		if (!response.ok) {
		  throw new Error(`HTTP error: ${response.status}`);
		}
						...
						var timer = new IntervalTimer(function () {
							...
							if (++ticksInCurrentBar === noise_loop) {
								...
								function transctBaseline(status, price){
									...
									amt_sell_time = crossover_emaoutput[counter_crossover].time_utc+" 1x - "+status+" Exit";
									amt_sell = price;
									amt_buy_time = entryTime+" - "+status+" - x1 ";
									...
									$("#time_update").html(crossover_emaoutput[counter_crossover].time_utc+" 1x "+status+" Entry");
									playPause();
								...
								if(signalShoww == "sell"){
									if(entryPrice != "" && tradeTransact == "1"){								
										...
										if(amt_sell > amt_buy){	
											...
											$(".pft-loss").removeClass("longBkg");
											$(".pft-loss").addClass("shortBkg");
											...
											$(".pft-loss").removeClass("shortBkg");
											$(".pft-loss").addClass("longBkg");
							...
							function shortTransc(status){
								...
								if(tradeEntryx3 != "" && tradeTransactx3 == "1"){
									...
									amt_sell_time_x3 = entryTime_x3+" x3 - Short";
									amt_buy_x3 = takeProfitx3;
									amt_buy_time_x3 = crossover_emaoutput[counter_crossover].time_utc+" x3 - Short Exit";
							...
							function longTransc(status){
								...
								if(tradeEntryx3 != "" && tradeTransactx3 == "1"){
									...
									entryTime_x3 = $("#time_updatex3").html()
									amt_buy_time_x3 = entryTime_x3+" x3 - Long";
									amt_sell_x3 = takeProfitx3;
									amt_sell_time_x3 = crossover_emaoutput[counter_crossover].time_utc+" x3 - Long Exit";
							...
							function TranscNoise(status, price){
								...
								amt_sell_time = entryTime+status+ " - x10 Exit";
								amt_sell = takeProfit;
								amt_buy_time = $("#time_update").html()+status+" - 10x Entry";
								if(typeof(amt_sell) === "undefined"){
									amt_sell = amt_buy;
									//alert("Sell == Buy: "+amt_buy)
								}
...
function longTrade(){
	...
	var tradedtls = "<td>"+symbl+"</td><td>Long</td><td>"+Tradebal+"</td><td>"+entry_price+"</td><td>"+stop_loss+"</td><td>"+take_profit+"</td><td class='pft-loss'>0%</td><td class='tradeModeBtn' onclick='tradeMode()'>Close Trade</td>";
