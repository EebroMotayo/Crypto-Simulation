...
var intervals = ['D','6h','4h','1h','30m','15m','5m'];
...
function syncToInterval(interval) {
	...
	if(interval == "D"){
		...
		if(chart2Data == "Open"){
			...
			fetch('http://127.0.0.1:5000/charthistory/'+symbl+'/'+interval)
			.then((r) => r.json())
			.then((response) => {
				//console.log(response)		
			...
			.then((response) => {
				...
				// #region Chandellier
				fetch('http://127.0.0.1:5000/chartz_runbot_chandelier/'+symbl+'/'+interval)
				.then((r) => r.json())
				.then((response) => {
				...
				for (var i = 2; i < datesForMarkers_cross.length; i++) {
					...
					if(datesForMarkers_cross[i].position == "buy" && markers_check_noise[0] != 'buy'){
						markers_noise.push({
							time: datesForMarkers_cross[i].time,
							position: 'belowBar',
							color: '#2196F3',
					...
					}else if(datesForMarkers_cross[i].position == "sell"  && markers_check_noise[0] != 'sell'){
				...
				candleSeries_hull.setMarkers(markers_noise);
		...
		if(chartData == "Open"){
			...
			candleSeries_hull_4hr.setMarkers(markers_noise);
		...
		if(chart4Data == "Open"){
			...
			candleSeries_hull_1hr.setMarkers(markers_noise);
		...
		if(chart5Data == "Open"){
			...
			candleSeries_30m.setMarkers(markers_noise);
		...
		if(chart6Data == "Open"){
			...
			candleSeries_15m.setMarkers(markers_noise);
		...
		if(chart7Data == "Open"){
			...
			candleSeries_5m.setMarkers(markers_noise);
...
function load_chart(){
	...
	for (var i = 2; i < datesForMarkers_cross.length; i++) {
		...
		if(datesForMarkers_cross[i].position == "buy" && markers_check[0] != 'buy'){
			markers.push({
				time: datesForMarkers_cross[i].time,
				position: 'belowBar',
				color: '#2196F3',
		...
		}else if(datesForMarkers_cross[i].position == "sell"  && markers_check[0] != 'sell'){
...
function load_atr(){
	...
	.then((response) => {
		...
		atr_output_2 = atrline;
			...
			var atr_crossover_trade1 = Number(Math.floor(atrline[lastcounter].value));
			...
			gen_tradeLines(markers_check[0]);
								...
								if (++ticksInCurrentBar === noise_loop) {
									...
									if(crossover_emaoutput[counter_crossover].time == data3[counter].time){
										...
										if(crossover_emaoutput[counter_crossover].position == "buy"){
											if(crossover_emaoutput[counter_crossover].position_status == "enter long"){
											...
											}else if(crossover_emaoutput[counter_crossover].position_status == "exit long"){
												markers.push({
													time: currentDay,
													position: 'aboveBar',
													color: '#9bfb83',
													shape: 'circle',
													text: data3[counter].close+' - Exit Long - ['+atr_crossover+']'
												});
												...
												// 	$(".tradeTransact").html("1");
												// 	$(".tradeTransactMode").html("1");
												// 	$("#time_update").html(crossover_emaoutput[counter_crossover].time_utc+" 1x Short Entry")
											...
											if(crossover_emaoutput[counter_crossover].position_status == "enter short"){
											...
											}else if(crossover_emaoutput[counter_crossover].position_status == "exit short"){
												markers.push({
													time: currentDay,
													position: 'belowBar',
													color: '#f57c69',
													shape: 'circle',
													text: data3[counter].close+' - Exit Short - ['+atr_crossover+']'
												});
												...
												// 	$(".tradeTransact").html("1");
												// 	$(".tradeTransactMode").html("1");
												// 	$("#time_update").html(crossover_emaoutput[counter_crossover].time_utc+" 1x Long Entry")
