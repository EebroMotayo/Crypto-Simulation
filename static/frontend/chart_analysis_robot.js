$("document").ready(function(){
	...
	//load_market_data();
	load_transaction();
	// syncToInterval, load_atr_0, gen_tradeLines, money_mngmt == "False"
	$(".order_status_btn").click(function(){
		// alert($(this).html());
		if(confirm("Do you want to change Order Status??")){
			change_order_status($(this).html())
		}
...
var watch_mode = $("#watch_threshold").html()
var watch_trade_mode = $("#watch_trade").html()
var watch_position = $("#watch_position").html()
var watch_trailing_stop = $("#watch_trailing_stop").html()
	...
	if(interval == "Trade"){
		...
		if(chartData0 == "Open"){
			...
			.then((response) => {
				...
				$("#gridline_direction").html(markers_base_check[0])
				//alert('direction')
				if(money_mngmt == "True" && watch_mode == "False"){
					gen_tradeLines(markers_base_check[0]);
				}else if(watch_mode == "True"){
					gen_tresholdLines()
				}
...
function load_atr_0(tmfrm){
	...
	.then((response) => {
		...
		var lastcounter = response.length-1
		var lastprice = parseFloat(response[lastcounter].close).toFixed(4)
		...
		var atr_crossover_trade1 = Number(parseFloat(response[lastcounter].value).toFixed(fixed_num));
		//var price1 = Number($("#price_update").html());
		var price1 = Number(parseFloat(response[lastcounter].close).toFixed(fixed_num));
		var avrAtr1 = parseFloat((atr_crossover_trade1/price1)*100).toFixed(fixed_num);
		var atrData1 = atr_crossover_trade1+" - x"+avrAtr1;
		var atrPrice = atr_crossover_trade1*3;
		take_prf = parseFloat(price1+atr_crossover_trade1*3).toFixed(fixed_num);
		stop_lss = parseFloat(price1-atr_crossover_trade1*1.5).toFixed(fixed_num);
		...
		if(markers_base_check[0] == 'Long'){
			take_prf = Number(parseFloat(atrPrice+price1).toFixed(fixed_num));
			avrAtr1_ls = Number(parseFloat(avrAtr1/2).toFixed(fixed_num));
			stop_ls = Number(parseFloat(atr_crossover_trade1*1.5).toFixed(fixed_num));
			stop_lss = Number(parseFloat(price1-stop_ls).toFixed(fixed_num));
			//alert("("+stop_lss+") - "+price1+" - "+stop_ls+" - Long || - "+baseline_check[0]+" | "+markers_check[0])

			//alert("atrPrice: "+atrPrice+" - Price:"+price1+" take_prf: "+take_prf)
			//alert("atr_crossover_trade1: "+atr_crossover_trade1+" - avrAtr1_ls:"+avrAtr1_ls+" stop_ls: "+stop_ls)
			//alert(atrPrice)
			...
			take_prf = Number(parseFloat(price1-atrPrice).toFixed(fixed_num));
			avrAtr1_ls = Number(parseFloat(avrAtr1/2).toFixed(fixed_num));
			stop_ls = Number(parseFloat(atr_crossover_trade1*1.5).toFixed(fixed_num))
			stop_lss = Number(parseFloat(price1+stop_ls).toFixed(fixed_num));
			...
			// $("#longSignal").removeClass()
			// $("#longSignal").addClass("sell")
			//alert("atrPrice: "+atrPrice+" - Price:"+price1+" - Stop Loss:"+stop_ls)
			
		}	
			//alert("("+stop_lss+") - "+price1+" - "+stop_ls+" - Short || - "+baseline_check[0]+" | "+markers_check[0])
		// }else if(markers_check[0] == 'sell'){
		// 	take_prf = parseFloat(price1-atr_crossover_trade1*3.5).toFixed()
		// 	stop_lss = parseFloat(price1+atr_crossover_trade1*1.5).toFixed()
		// }
		...
		//alert(stop_lss+" - "+price1+" - "+take_prf)

		$("#price_update").html(price1)
		$(".price_updt").html(price1)
...
function tradeModeEMA(){
	...
	if(loadclose == "open" || loadclose == "loaded"){
		...
		if(signalMode_spot == "1"){
			...
			$("#watch_trade").html("do")
			if(loadclose == "loaded" && tradestatus == "True"){
				change_spot_trading("False");
			}
			...
			$("#watch_trade").html("")
			if(loadclose == "loaded" && tradestatus == "False"){
				change_spot_trading("True");
			}
...
function openTradebox(entry_price, take_profit, stop_loss, atr_crossover_trade){
	...
	var direction = $("#gridline_direction").html();
	...
	if(direction == "Long"){
		...
		var pcentProfitbal = parseFloat(take_profit-entry_price).toFixed(fixed_num);	
		var pcentLosstbal = parseFloat(entry_price-stop_loss).toFixed(fixed_num);
		...
		var pcentProfitbal = parseFloat(entry_price-take_profit).toFixed(fixed_num);
		var pcentLosstbal = parseFloat(stop_loss-entry_price).toFixed(fixed_num);
		var pcentProfit = parseFloat((pcentProfitbal/entry_price)*100).toFixed(2)
		var pcentLoss = parseFloat((pcentLosstbal/entry_price)*100).toFixed(2)
...
function longTrade(){
	...
	if(order_status == "Continuous"){
		status_class = "contdtrd"
	}else if(order_status == "One-Time"){
		status_class = "onettrd"
	}
	
	//alert("Buy - entry price - "+entry_price +"Buy take profit - "+ take_profit+"Buy stop loss - "+stop_loss)

	var tradedtls = "<td>"+symbl+"</td><td class='pos_type buytrd'>Long</td><td id='order-st' class='"+status_class+"'><span class='order_status_btn'>"+order_status+"</span></td><td>"+Tradebal+"</td><td>"+entry_price+"</td><td class='price_updt'>"+price+"</td><td>"+stop_loss+"</td><td>"+take_profit+"</td><td class='pft-loss'>0%</td>";
...
function shortTrade(){
	...
	var tradedtls = "<td>"+symbl+"</td><td class='pos_type selltrd'>Short</td><td id='order-st' class='"+status_class+"'><span class='order_status_btn'>"+order_status+"</span></td><td>"+Tradebal+"</td><td>"+entry_price+"</td><td class='price_updt'>"+price+"</td><td>"+stop_loss+"</td><td>"+take_profit+"</td><td class='pft-loss'>0%</td>";
...
function modifyTrade(){
	...
	var tradedtls = "<td>"+symbl+"</td><td class='pos_type "+bgCol+"'>Long</td><td id='order-st' class='"+status_class+"'><span class='order_status_btn'>"+order_status+"</span></td><td>"+Tradebal+"</td><td>"+entry_price+"</td><td class='price_updt'>"+price+"</td><td>"+stop_loss+"</td><td>"+take_profit+"</td><td class='pft-loss'>0%</td>";
...
function closeTrade(){
	...
	//alert(order_status)
	var tradedtls = "<td>"+symbl+"</td><td class='pos_type closetrd'>Close</td><td id='order-st' class='"+status_class+"'><span class='order_status_btn'>--</span></td><td>"+Tradebal+"</td><td>"+entry_price+"</td><td class='price_updt'>"+price+"</td><td>"+stop_loss+"</td><td>"+take_profit+"</td><td class='pft-loss'>0%</td>";
...
function gen_tradeLines(direction){
	...
	if(loadlines == "loaded"){
		$("#gridline_direction").html(direction)
...
function gen_tresholdLines(){
	...
	watch_mode = $("#watch_threshold").html()

	//alert("watch_mode: "+watch_mode+" - loadwatch: "+loadwatch)

	if(watch_mode == "True" && loadwatch == ""){
		...
		var upperPrice = parseFloat(Number(avgPrice+atrData*1.5)).toFixed(fixed_num)
		var lowerPrice = parseFloat(Number(avgPrice-atrData*1.5)).toFixed(fixed_num)
	}
	...
	var lowerColor = "#a9fc00";
	var upperTitle = "Reverse Trigger";
	var lowerLower = "Continuation Trigger";
	...
	if(markers_base_check[0] == 'Long'){
		...
		if(watch_mode == "True"){
			upperColor = "#03f9fb";
			lowerColor = "#a9fc00";
			upperTitle = "Reverse Trigger";
			lowerLower = "Continuation Trigger";
		}
		...
		if(watch_mode == "True"){
			upperColor = "#a9fc00";
			lowerColor = "#03f9fb";
			lowerLower = "Reverse Trigger";
			upperTitle = "Continuation Trigger";
		}
		
	}

	//alert("upperPrice - buy: "+upperPrice+" - lowerPrice - sell: "+lowerPrice)
	...
	if(loadwatch == "loaded" || watch_mode == "False"){
		do_watch_line("True")
	}
...
function handler2(params) {
	...
	if(line.options().title == "Entry Order"){
		...
		// var price_mod2 = chart_sma.priceScale('right').formatPrice(line.options());
...
function save_market_data(){
	...
	var tradestatus = $("#tradestatus").html();
	var order_status = $(".order_status").html();
	direction = $("#gridline_direction").html();
	//alert("entry price - "+entry_price +" take profit - "+ take_profit+" stop loss - "+stop_loss)
	//exit()
	//alert(order_status);
	//alert(direction);
...
function change_order_status(status){
    ...
    $.ajax({
        type: "POST",
        url: url_link+"/change_order_status",        
        contentType: "application/json",
        dataType: "json",  
        data: JSON.stringify(dataz),      
        success: function(response){
			...
			if(response['output'] == 'success'){
				$(".notfc").slideDown();
				$(".notfc strong").html("Order Status Updated");
				setTimeout(function () {
					$(".notfc").slideUp();
					$(".notfc strong").html("");
				}, 3000);
				if(status == "Continuous"){
					$(".order_status").html("One-Time")
					$(".order_status_btn").html("One-Time")
					$("#order-st").removeClass("contdtrd")
					$("#order-st").addClass("onettrd")
				}else if(status == "One-Time"){
					$(".order_status").html("Continuous")
					$(".order_status_btn").html("Continuous")
					$("#order-st").removeClass("onettrd")
					$("#order-st").addClass("contdtrd")
				}else{
					$(".order_status").html("Continuous")
					$(".order_status_btn").html("Continuous")
					$("#order-st").removeClass("onettrd")
...
function changecoin(status){
	...
	var url_link = "";
	var online_status = $("#online_status").val();
	if(online_status == "True" && status == "save"){
		url_link = $("#server_link").val();
	}

	// if(status == "load"){
	// 	changeServerLink("", "off-L");
	// }
...
function change_spot_trading(trademode){
    ...
    $.ajax({
        ...
        success: function(response){
			...
			if(response['output'] == 'success'){
				$(".notfc").slideDown();
				$(".notfc strong").html("Successfully Change Spot Trading");
				setTimeout(function () {
					$(".notfc").slideUp();
					$(".notfc strong").html("");
				}, 3000);

				$("#tradestatus").html(trademode);
			}
...
function do_watch_line(status){

	// var threshold = $("#watch_threshold").html();
	var tmfrm = $("#timeframe_backtest").html();
...
function calcRealtimeProftLossInit(){

	entry_price = Number($("#orderEntry").val());
	take_profit = Number($("#Takeprofit").val());
	stop_loss = Number($("#stopLoss").val());
...
function calcRealtimeProftLoss(cur_price){
	...
	var priceUpdate = parseFloat(cur_price).toFixed(fixed_num)

	$(".price_updt").html(priceUpdate);
	$("#price_update").html(priceUpdate);
