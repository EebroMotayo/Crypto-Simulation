...
function syncToInterval(interval) {
	...
	if(interval == "Simulation"){
		...
		if(chartData0 == "Open"){
			...
			candleSeries_1 = chart_sma.addCandlestickSeries({
				upColor: '#00ff00',
				downColor: '#ff0000', 
			...
			.then((response) => {
				...
				candleSeries_1.setData(response);
			...
			// #region Chandellier
				...
				.then((response) => {
					...
					candleSeries_1.setMarkers(markers);
		...
		if(chartData == "Open"){
			...
			fetch('http://127.0.0.1:5000/chartz_runbot_ut_bot_alert/'+symbl+'/'+timeFrm2)
				.then((r) => r.json())
				.then((response) => {
					...
					candleSeries_hull.setMarkers(markers);
...
function load_atr(){
	...
	fetch('http://127.0.0.1:5000/sim_runbot_atr_period/'+symbl+'/'+tmfrm)
	.then((r) => r.json())
	.then((response) => {
...
function load_ut_bot_alert_baseline(){
	...
	fetch('http://127.0.0.1:5000/sim_baseline_runbot_ut_bot_alert/'+symbl+'/'+tmfrm)
	.then((r) => r.json())
	.then((response) => {
...
function load_ut_bot_alert(){
	...
	fetch('http://127.0.0.1:5000/sim_runbot_ut_bot_alert/'+symbl+'/'+tmfrm)
	.then((r) => r.json())
	.then((response) => {
...
function load_ut_bot_alert_noise(){
...
function playPrice2(){
	...
	chart_sma = LightweightCharts.createChart(document.getElementById('chart0'), {
		width: chartwidth,
		height: 505,
		layout: {
			backgroundColor: '#000000',
	...
	crossover_emaoutput = load_ut_bot_alert();
	crossover_baseline = load_ut_bot_alert_baseline();
	fastNoise_emaoutput = load_fastNoiseEMA();
	slowNoise_emaoutput = load_slowNoiseEMA();
	crossoverNoise_emaoutput = load_ut_bot_alert_noise();
			...
			.then((data) => {	
				...
				fetch('http://127.0.0.1:5000/chartsimulate/'+symbl+'/'+tmfrm)
					...
					.then((response) => {
						...
						var counter_crossover_baseline = 0;
						...
						var take_profit_update = $("#take_profit_update").html();
						var stop_loss_update = Number($("#stop_loss_update").html());
						var watch_mode = $("#threshold").html()
						var watch_trade_mode = $("#watch_trade").html()
						var watch_line_trade = $("#watch_line_trade").html()
						var watch_position = $("#watch_position").html()
						//#endregion
						...
						var timer = new IntervalTimer(function () {
							...
							if(slowNoise_emaoutput != ""){
								...
								//#region Trade Lines								
								function tradeLines(avgPrice, minimumPrice, maximumPrice){
								...
								//#region ATR Update
								if(counterAtr > 24) {
									var atr_crossover_trade1 = Number(Math.floor(atr_output[counterAtr].value));
									...
									}else if(baseline_check[0] == 'short'){
								...
								if (++ticksInCurrentBar === noise_loop) {
									...
									if(counter == counterLine){
										...
										function handler(params) {
											...
											if(line.options().title == "Entry Order"){
												...
												price = chart_sma.priceScale('right').formatPrice(line.options().price);
												...
												take_prf = chart_sma.priceScale('right').formatPrice(line.options().price);
												
											}else if(line.options().title == "Stop Loss"){

												stop_lss = chart_sma.priceScale('right').formatPrice(line.options().price);
												
											}
											
											//openTradebox(price, take_prf, stop_lss, atrData);
											removeTradeline()
											tradeLines(price, stop_lss, take_prf)

											// signalShoww = $(".signalShow").html();
											// if(signalShoww == 'buy'){
											// 	longTrade();
											// }else if(signalShoww == 'sell'){
											// 	shortTrade();
											// }
									...
									// console.log("cross: "+crossover_emaoutput[counter_crossover].time+" - data: "+data3[counter].time)
									// var tradeTransactMode = $(".tradeTransactMode").html();

									var tradeTransact = $(".tradeTransact").html();
									var TradeModeEMA = $(".TradeModeEMA").html();
									watch_mode = $("#threshold").html()
									watch_trade_mode = $("#watch_trade").html()
									watch_position = $("#watch_position").html()

									if(crossover_baseline[counter_crossover_baseline].time == data3[counter].time){

										//alert(watch_mode)

										var atr_crossover = Math.floor(atr_output[counterAtr].value);
										currentDay_baseline = timeConverter(data3[counter].time);
										currentDay2_baseline = crossover_baseline[counter_crossover_baseline].time_utc;
										// tradeTransact = $(".tradeTransact").html();
										// TradeModeEMA  = $(".TradeModeEMA").html();

										if(crossover_baseline[counter_crossover_baseline].position == "buy"){
											
											markers.push({
												time: currentDay_baseline,
												position: 'belowBar',
												color: '#2196F3',
												shape: 'arrowUp',
												text: 'Baseline @ ' + data3[counter].close+' - Long - ['+atr_crossover+']'
											});

											baseline_check.pop()
											baseline_check.push('long')

											take_profit_update = $("#take_profit_update").html();
											stop_loss_update = $("#stop_loss_update").html();

											
											//alert("tradeTransact_base: "+tradeTransact+" - TradeModeEMA_base: "+TradeModeEMA+" -- "+markers_check[0])

											if( watch_mode == "Inactive"){
												if(markers_check[0] == 'buy'){
												...
												$("#longSignal").addClass("buy")
													...
													atr_crossover_trade1 = Number(Math.floor(atr_output[counterAtr].value));
													avrAtr1 = parseFloat((atr_crossover_trade1/data3[counter].close)*100).toFixed(2);
													atrPrice = atr_crossover_trade1*avrAtr1;
													take_prf = parseFloat(atrPrice+data3[counter].close).toFixed(2)
													avrAtr1_ls = parseFloat(avrAtr1/2).toFixed(2)
													stop_ls = parseFloat(atr_crossover_trade1*avrAtr1_ls).toFixed(2)
													stop_lss = parseFloat(Number(data3[counter].close)-Number(stop_ls)).toFixed(2)
													removeTradeline()
													tradeLines(data3[counter].close, stop_lss, take_prf)
													longTrade();
										...
										}else if(crossover_baseline[counter_crossover_baseline].position == "sell"){
											...
											markers.push({
												...
												color: '#e91e63',
												shape: 'arrowDown',
												text: 'Baseline @ ' + data3[counter].close+' - Short - ['+atr_crossover+']'
											});

											baseline_check.pop()
											baseline_check.push('short')
											...
											if( watch_mode == "Inactive"){
												if(markers_check[0] == 'sell'){
													...
													$("#longSignal").addClass("sell")
														...
														avrAtr1 = parseFloat((atr_crossover_trade1/data3[counter].close)*100).toFixed(2);
														atrPrice = atr_crossover_trade1*avrAtr1;
														take_prf = parseFloat(data3[counter].close-atrPrice).toFixed(2)
														avrAtr1_ls = parseFloat(avrAtr1/2).toFixed(2)
														stop_ls = parseFloat(atr_crossover_trade1*avrAtr1_ls).toFixed(2)
														stop_lss = parseFloat(Number(data3[counter].close)+Number(stop_ls)).toFixed(2)
														removeTradeline()
									...
									if(crossover_emaoutput[counter_crossover].time == data3[counter].time){
										...
										if(crossover_emaoutput[counter_crossover].position == "buy"){
											...
											//alert("tradeTransact: "+tradeTransact+" - TradeModeEMA: "+TradeModeEMA+" -- "+baseline_check[0])
											...
											if( watch_mode == "Inactive"){
												...
												if(baseline_check[0] == 'long'){
													...
													//alert(data3[counter].close+" - Entry Long Market")
													$(".tradeEntry").html(data3[counter].close);
													$(".tradeTransact").html("1");
													$(".tradeTransactMode").html("1");
													$("#time_update").html(crossover_emaoutput[counter_crossover].time_utc)
													...
													//alert(data3[counter].close+" - Exit Short")
													$(".tradeExit").html(data3[counter].close);
													//$(".tradeTransact").html("0");
													$(".tradeTransactMode").html("1");
													//alert("price - "+data3[counter].close)
										...
										}else if(crossover_emaoutput[counter_crossover].position == "sell"){
											...
											if( watch_mode == "Inactive"){
												...
												if(baseline_check[0] == 'short'){
													...
													$("#longSignal").removeClass()
													$("#longSignal").addClass("sell")
													//alert(data3[counter].close+" - Entry Short Market")
												...
												}else if(baseline_check[0] == 'long'){

													//alert(data3[counter].close+" - Exit Long")													
													$(".tradeExit").html(data3[counter].close);
													//$(".tradeTransact").html("0");
													$(".tradeTransactMode").html("1");
													//alert("price - "+data3[counter].close)
											...
											// if(tradeTransact == "0" && TradeModeEMA == "1" && baseline_check[0] == 'short'){
											...
											// }else if(TradeModeEMA == "1" && baseline_check[0] == 'long'){
									...
									if(watch_trade_mode == 'do'){

										if(watch_position == 'buy'){

											$(".tradeExit").html(data3[counter].close);									
											$(".tradeTransactMode").html("1");										
											transctBaseline("Long", data3[counter].close);

										}else if(watch_position == 'sell'){
								...
								function calcProftLoss(){
									...
									if(signalShoww == "sell" && watch_mode == "Inactive"){
										//alert(entryPrice+" - "+tradeTransact)
									...
									}else if(signalShoww == "buy" && watch_mode == "Inactive"){
										//alert(entryPrice+" - "+tradeTransact)
										if(entryPrice != "" && tradeTransact == "1"){	
											//alert(entryPrice);								
											amt_buy = entryPrice;
								...
								if(counter > counterPricecheck && watch_mode == "Inactive"){
									signalShoww = $(".signalShow").html();
									entryPrice = $("#orderEntry").val();
									takeProfitx3 = $("#Takeprofit").val();
									stop_loss = $("#stopLoss").val();
									var signalMode = $(".TradeMode").html();
									watch_line_trade = $("#watch_line_trade").html()
									//alert(stoploss+" - "+signalMode)
									if(signalShoww == "buy" && signalMode == "1"){
										...
										if(noise_response[counter_noise].low <= stop_loss){
											//alert(signalShoww+': Loss|| '+noise_response[counter_noise].low +" - "+stop_loss)
											longTransc("Loss");
										...
										if(noise_response[counter_noise].high >= takeProfitx3  ){

											//alert(signalShoww+': Profit|| '+noise_response[counter_noise].high +" - "+takeProfitx3)
											longTransc("Profit");
											$(".tradeTransactx3").html("0");
											//playPause();
										}
										...
										if(noise_response[counter_noise].high >= stop_loss){
											//alert(signalShoww+': Loss|| '+noise_response[counter_noise].high+" - "+stop_loss)
											shortTransc("Loss");
											$(".tradeTransactx3").html("0");
										...
										if(noise_response[counter_noise].low <= takeProfitx3){
	
											//alert(signalShoww+': Profit|| '+noise_response[counter_noise].low+" - "+takeProfitx3)
											shortTransc("Profit");
											$(".tradeTransactx3").html("0");
											if(crossover_emaoutput[counter_crossover].position_status == "exit long" || crossover_emaoutput[counter_crossover].position_status == "exit short"){
												//playPause();
									...
									if(watch_line_trade == "do"){

										if(signalShoww == "buy"){

											//alert("jjj "+crossover_emaoutput[counter_crossover].position_status)
											if(noise_response[counter_noise].low <= entryPrice){
												//alert(signalShoww+': Loss|| '+noise_response[counter_noise].low +" - "+stop_loss)
												longTransc("Loss");
												$(".tradeTransactx3").html("0");
												//playPause();
											...
											if(noise_response[counter_noise].high >= entryPrice  ){
...
function gen_tradeLines(position){

	$(".signalShow").html(position);
