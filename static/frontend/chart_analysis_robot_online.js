...
var current_pair = $("#current_pair").html();
...
function syncToInterval(interval) {
	...
	if(interval == "Trade"){
		...
		if(chart2Data == "Open"){
			...
			fetch('http://127.0.0.1:5000/chartz_runbot_ut_bot_alert_baseline/'+symbl+'/'+interval)
			.then((r) => r.json())
			.then((response) => {
...
function transactionValues(trade_details){
	...
	var dt = `<tr>
				...
				<td style="width: 150px;text-align: center;color:${bgcolor}">${rec_track_profit}/${rec_track_loss}</td>
				<td style="width: 150px;text-align: center;color:#d98b8b">${rec_track_loss_value}</td>
			</tr>`;
...
function transactionDetails(trade_details){
	...
	current_pair = $("#current_pair").html();
	...
	for (var i = j; i > 0; i--) {
		...
		stop_loss = trade_details[i].stop_loss;
		take_prf = trade_details[i].take_profit;

		if(trade_details[i].pair == current_pair){
			if(trade_details[i].profit_loss < 0){
			...
			if(trade_details[i].position_type == "Short"){
				stop_loss = trade_details[i].take_profit;
				take_prf = trade_details[i].stop_loss;
			}
			...
			var dt = `<tr>
						...
						<td style="width: 150px;text-align: center;">${stop_loss}</td>
						<td style="width: 150px;text-align: center;">${take_prf}</td>					
...
function longTrade(){
	...
	var tradedtls = "<td>"+symbl+"</td><td>Long</td><td>"+Tradebal+"</td><td>"+entry_price+"</td><td class='price_updt'>"+price+"</td><td>"+stop_loss+"</td><td>"+take_profit+"</td><td class='pft-loss'>0%</td>";
...
function closeTrade(){
	...
	//alert(tradestatus)
	var tradedtls = "<td></td><td></td><td></td><td></td><td class='price_updt'></td><td></td><td></td><td class='pft-loss'></td>";
...
function gen_tresholdLines(){
	...
	$("#watch_position").html(signalShow)
	$("#watch_trade").html("do")
	$("#LoadTrade").html("");
	clean_tradeLines()
	signalMode = $(".TradeMode").html();
